...
distinction between the extensional and intensional dimensions of the context is largely a matter of convention, or a means of improving intuition when one is first introduced to FCA. There is no strict requirement around what these sets are made up of, or
...
The absence of an object-attribute pair from the relation is not usually interpreted to mean that the object has the negation of the attribute.
...
It is often enough to study a restriction of attribute logic, which limits the expressivity in a way that is analogous to
propositional Horn logic. Much of the interest in this restriction has to do with FCA-style \textit{implications}:
...
The definition for when a context respects an implication admits a considerably more concise representation; it is equivalent
to the expression that $A^{\downarrow}\subseteq B^{\downarrow}$, or that \say{All those objects satisfying (all) attributes in $A$ also satisfy (all) attributes in $B$.}
...
\Cref{proposition:implication-concepts} suggests that the concept lattice may be used to determine whether an
implication holds in its' respective context.

...
The introduction of models at the end of \Cref{section:propositional-logic} leads quite naturally into a discussion on the matter of \textit{logical consequence}, which concerns how a formal system might be developed which remains compatible with the common usage of \textit{consequence} \cite[p. 408]{turquette1957logic}.  
...
Logical consequence---as described in \Cref{subsection:logical-consequence}---offers a purely semantic perspective on how we might justify that a certain inference follows (logically) from another formula, or set thereof. In contrast, deductive systems are a purely syntactic alternative for defining when an inference can be made. 

Generally, these systems are comprised of \textit{axioms} and \textit{rules of inference} which are chained together in a finite sequence to construct a \textit{proof}. In such a sequence, every element is either an axiom of the system, or can be inferred by application of an inference rule to an earlier element. The final element of such a valid sequence is called a \textit{theorem}. 
...
As an illustration, we provide the following Hilbert-style proof sequence for the inference made in \Cref{example-logical-consequence}. 
...
\begin{proofH}
  \Hstep[Premise]{(\texttt{b} \rightarrow \texttt{h})}
  \Hstep[Premise]{(\texttt{h} \rightarrow \texttt{s})}
  \Hstep[Axiom 1]{(\texttt{h} \rightarrow \texttt{s}) \rightarrow (\texttt{b} \rightarrow (\texttt{h} \rightarrow \texttt{s}))}
  \Hstep[MP 2,3]{(\texttt{b} \rightarrow (\texttt{h} \rightarrow \texttt{s}))}
  \Hstep[Axiom 2]{(\texttt{b} \rightarrow (\texttt{h} \rightarrow \texttt{s})) \rightarrow ((\texttt{b} \rightarrow \texttt{h}) \rightarrow (\texttt{b} \rightarrow \texttt{s}))} 
  \Hstep[MP 4,5]{(\texttt{b} \rightarrow \texttt{h}) \rightarrow (\texttt{b} \rightarrow \texttt{s})}
  \Hstep[MP 1,6]{(\texttt{b} \rightarrow \texttt{s})}
\end{proofH} 
...
\index{soundness} \index{completeness}

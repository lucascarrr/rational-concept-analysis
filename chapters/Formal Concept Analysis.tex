...
This section is concerned with the discussion of \textit{Contextual Attribute Logic(s)} \cite{ganter2025language} which
lie under Wille's \textit{Contextual Logic} \shortcite{Dau2005}.

\section{Attribute Implications}
\label{section:attribute-implications}

There is another side to FCA, wh
...
\texttt{immigration }$. If we examine the context in \Cref{figure:voting-records}it is apparent that every representative
...
We distinguish between compound and ``normal'' attributes by using lowercase Greek letters for compound attributes, and lowercase
...
In propositional logic, we might construct a formula by taking the conjunction operation over a set of formulae. It is, however,
...
\Cref{chapter:defeasible-reasoning-in-fca}, we rely on this finiteness property holding in compound attributes, and so

...
We begin the introduction of KLM style defeasible reasoning to FCA by defining an analogue to preferential interpretations.
...
states have been omitted from our definition. Formal contexts have no restriction on including two objects with equivalent intension, meaning that states seem to be implicitly constructed in
contexts.
...
\begin{definition}
	...
	\]
\end{definition}
The ordering indicates that the member of Congress designated by \texttt{moc-0} is preferred, under \textit{some} metric, to all other members. They are a Republican and could thus be considered
the most typical Republican. It is a subtle point that, in spite of their status as being the most preferred Republican, their minimal position need not be attributed to some Republican-specific
feature. For instance, it might be that members of congress who voted in favour of the \texttt{easa} bill are preferred, irrespective of party affiliation. In this sense, \texttt{moc-0} being
the most preferred Republican is a by-product of another preference. For the time being we remain agnostic as to \textit{how} one may construct such a preference relation in general, this
...
attribute set $M$, and so the expression $A \twiddle B$ should be understood to be saying that the ``most preferred'' objects in a context that have all attributes from $A$ also have all attributes
from $B$.
...
A set of conditionals that is contextually complete to a ranked context $\mathbb{R}$ and constraint set $\Delta$ provides abstracted perspective, in terms of defeasible conditionals, on the information
contained in the pair $(\mathbb{R}, \Delta )$. It is now the goal, as was done in \Cref{subsubsection:implication-bases}, to find such a set that is non-redundant, or the \textit{rational

...
The conlcusion of \Cref{part:1} marks the end of the scaffolding component of this work. The remaining
...
progresses, the setting of FCA may be quite naturally extended to accomodate the necessary semantics.
...
We begin this unification by providing justification for why non-monotonicity is beneficial in FCA at all.
There are certain properties of FCA which are substantially undermined by the violation of monotonicity. For
instance, a central notion of a concept lattice is that it describes chains of inheritance between concepts
and their sub-concepts. The ability to preserve these becomes tenuous under non-monotonic semantics, as it
requires transitivity, which we have already shown is not a property of KLM style reasoning.

Nonetheless, there are indeed settings where the enforcement of monotonicity has similarly undesirable
second-order effects. Many of these settings are characterised by arguments similar to those given in
\Cref{section:nmr-background}: that there are exceptional instances of a concept, or more generally some
\textit{class}, and that it is often useful to be able to put aside the exceptions and reason prototypically
with respect to the considered concept. Another consideration in FCA is how one might represent prototypical
concepts within the concept lattice, but this discussion will be left to the next chapter for which it is
more pertinent. The argument we consider here is borne out of the recognition that FCA is frequently used as
...
used to provide a semantics for defeasible attribute implications, which we refer to as \textit{defeasible
	conditionals}. If $X,Y \subseteq M$ are sets of attributes, then the expression $X \twiddle Y$ tells us that
the typical extension of $X$ is contained in the extension of $Y$.
...
\begin{definition}
	...
	Let $\pcontext = \PGMI$ be a preferential context, $A,B \subseteq M$ subsets of attributes, and $A \twiddle
		B$ \emph{defeasible conditional}. We say that $A \twiddle B$ is \emph{satisfied} in $\pcontext$ if and only
...
are members of the derivation of $. For those with a background in FCA, satisfaction may be expressed more
	familiarly as \subseteq \Minr{A}$.

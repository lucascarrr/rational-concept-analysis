...
In other words, a closure system $\mathcal{C}$ of a set $S$ is a subset of the powerset of $S$, such that the intersections of any elements of $\mathcal{C}$ (which are all closed sets) is closed themself. From \Cref{example:power-set-lattice}, we know that under the inclusion order, the powerset forms a complete lattice; from the previous sentence it follows that a closure system on $S$ is a meet subsemilattice of $\pset{S}$.
% In essence, closure systems are collections of sets that remain closed under arbitrary intersections. This property reflects a fundamental principle: if multiple sets each satisfy a closure property, their intersection also satisfies that property. This seemingly abstract condition has profound implications for the structure of closed sets in mathematical systems

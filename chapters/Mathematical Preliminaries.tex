...
\label{definition:closure-system}
	Given a set $X$, a \emph{closure system} on $X$ is a set of subsets containing $X$ that is closed under intersection. That is, $Y \subseteq \pset{X}$ is a closure system on $X$ if $X \subseteq Y$ and $Z \subseteq Y$ implies $\bigcap Z \in Y$.
\end{definition}

\begin{definition}
\label{definition:closure-operator}
	A \emph{closure operator} on a set $X$ is a function $\bullet \colon \pset{X} \to \pset{X}$, assigning each subset of $X$ to its' closure $\bullet X \subseteq X$, that satisfies:
	\begin{align}
    \text{(Monotony)} \quad & X \subseteq Y \text{ implies } \bullet X \subseteq \bullet Y \\
    \text{(Extensivity)} \quad & X \subseteq \bullet X \\
    \text{(Idempotency)} \quad & \bullet X = \bullet \bullet X
  \end{align}
\end{definition}


\begin{theorem}
\label{theorem:closures}
	If $\mathcal{C}$ is a closure system on $X$ then $\bullet X \mapsto \pset{X}$
\end{theorem}

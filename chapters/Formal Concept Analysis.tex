...
If we consider the powersets $\pset{G}$ and $\pset{M}$ with their usual inclusion orders, the \Cref{equation:galois-1,equation:galois-2,equation:galois-3} are merely rephrasings of \Cref{equation:ord_galois-1,equation:ord-galois-2,equation:ord-galois-3}; and \Cref{equation:galois-4} rephrases \Cref{proposition:fundemental-galois}, and so, the derivation operators form a Galois connection between the powersets $\pset{G}$ and $\pset{M}$. We remind the reader of \Cref{theorem:powerset-lattice}, from which it it follows that $\pset{G}$ and $\pset{M}$ are both complete lattices.

Let us now consider some intution for why Galois connections appear when modelling concepts as the dual between extension and intension.

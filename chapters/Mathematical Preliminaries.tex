...
When studying a collection of certain \textit{things} that satisfy a certain property, it is often beneficial to know if the collection considers every item satisfying the specified properties. We aliken this to the more formal notion of \textit{closure} and \textit{closed sets}.

\begin{definition}
\label{definition:closed-set}
	A subset $S$ of $X$ is \textit{closed} with respect to an operation $\mathsf{f}$ if and only if the application of $f$ to elements of $S$ always results in an element of $S$. Formally, $\mathsf{f}(S) = S$
\end{definition}

Closely related is the notion of \textit{closure operators}
%
\begin{definition}
\label{definition:closure-operator}
	A \emph{closure operator} on a set $S$ is a function $\mathsf{cl} \colon \pset{S} \to \pset{S}$ that satisfies the following properties for all $X,Y \subseteq S$.
	\vspace{-1em}
	\begin{align}
  ...
  The subsets $X \subseteq S$ where $X = \mathsf{cl}(X)$ are called \textit{closed} subsets of $S$ with respect to $\mathsf{cl}$.
\end{definition}
%
More generally, we say that a set $S$ is closed with respect to an operation $\mathsf{f}$ if applying $\mathsf{f}$ to members of $S$ always produces another member of $S$. A simple example is that the the natural numbers are closed under multiplication, but not division, as the product of two natural numbers is always another natural number, while the quotient is not.

\begin{definition}
\label{definition:closure-system}
Given a set $S$, a \emph{closure system} on $S$ is a set of subsets $\mathcal{C} \subseteq \pset{S}$ that is closed under intersection. In other words, if $X \subseteq \mathcal{C}$ then $\bigcap X \in \mathcal{C}$.
\end{definition}

Given a set $X$ the closure of $X$ can be constructed by taking $\bigcap \{Y \in \pset{S} \mid X \subseteq Y \}$
...
\begin{tikzpicture}[
    every node/.style={
     ...
     fill=black,
      text=black,
      inner sep=0pt,          % no extra padding
      minimum size=2mm,       % ← choose one diameter for all nodes
      font=\scriptsize   % smaller label size → smaller overall node
    }]
   ...
   \draw[color=red, ->, dotted] (6)to[out=350,in=-100] (1);
   \draw[color=red, ->, dotted] (5)to[out=350,in=-100] (4);
\end{tikzpicture}
\end{figure}

...
Concerns over the brittleness of---as well as the benefits of relaxing---the semantics of (classical) attribute implications are well known in the FCA literature, and in data mining more generally
\cite{Lakhal2005}. \textit{Association rules} are able express implication-like statements despite the existence of counterexamples in the data, or in our case a context. These are statements
like \say{$66\%$ of the members of congress who were \textit{Democrats}, which makes up $50\%$ of congress, voted on the \textit{MX-Missile} bill}.
...
as possible, resulting in some restrictions of the analogy to preferential reasoning. As we progress, we discuss the amendments necessary to lift these restrictions. The idea behind this order
of exposition is that we view this work as largely an extension to FCA rather than addition to the KLM area; and so, there may be uses of this kind of defeasible reasoning in FCA which do not
wish to depart too much from the original setting.
...
Preferential contexts, albeit with a different motivation, were first introduced by Obiedkov \shortcite{Obiedkov2012}, and later on with our purposes in mind in \cite{Carr2024}. An example of
what such a preference order may look like for the context in \Cref{figure:voting-records} is given below.
%
\begin{figure}[H]
	\centering
...
\end{figure}
%
Preferential interpretations were defined by a relation on states which were then mapped to valuations, with no requirement that the mapping be injective. It may then be surprising that
states seem to have been entirely omitted from our definition. The reason being that formal contexts have no restriction on the inclusion of two objects with an equivalent intension, meaning
...
If we considered the attribute $\{\, \texttt{rep}\,\}$, its' extent contains the objects $\{\,\texttt{moc-0,moc-1,moc-79}\,\}$ and the respective minimisation is the single object $\{ \, \texttt
{moc-0}\,\}$. In some sense this conveys that \texttt{moc-0} should be regarded as the most preferred Republican. It is a subtle point that, in spite of this status, their minimal position
need not be attributed to some Republican-specific feature. For instance, it might be that members of congress who voted in favour of the \texttt{easa} bill are preferred, irrespective of
party affiliation. In this sense, \texttt{moc-0} being the most preferred Republican is a by-product of another preference. It makes little practical difference, but is possibly important for
ones' intuition of how the preference relation was constructed; a matter which, for the time being, we remain agnostic to. This will be the subject of \Cref{subsubsection:finding-order}.
...
set $M$, and so the expression $A \twiddle B$ should be understood to be saying that the ``most preferred'' objects in a context that have all attributes from $A$ also have all attributes
from $B$.
